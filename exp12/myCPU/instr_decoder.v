...
assign alu_op = {
    ...
    inst_slt | inst_slti,
    inst_sub_w,
    inst_add_w | inst_addi_w | inst_ld_w | inst_st_w | inst_jirl | inst_bl | inst_pcaddu12i | inst_ld_b | inst_ld_h | inst_ld_bu | inst_ld_hu | inst_st_b | inst_st_h   // op_add
};
...
assign sel_alu_src2[3] = need_ui12              ;
assign sel_alu_src2[2] = need_si20              ;  // lu12i
assign sel_alu_src2[1] = src2_is_4              ;  // bl || jirl
assign sel_alu_src2[0] = need_ui5 || need_si12  ;  // 复用

assign imm = {32{sel_alu_src2[3]}} & {20'b0, i12[11:0]}         |
             {32{sel_alu_src2[2]}} & {i20[19:0], 12'b0}         |
             {32{sel_alu_src2[1]}} & 32'h0000_0004              |
             {32{sel_alu_src2[0]}} & {{20{i12[11]}}, i12[11:0]} ;
...
assign sel_nextpc[1] = inst_beq | inst_bne | inst_b | inst_bl | inst_blt | inst_bge | inst_bltu | inst_bgeu;
assign sel_nextpc[2] = inst_jirl;

// Comparator
assign {adder_cout, adder_result} = {1'b0, rj_value} + {1'b0, ~rkd_value} + 1'b1;  // 不做33-bit扩展有截断风险，逻辑对但仿真不过

assign rj_eq_rd = (rj_value == rkd_value);

assign rj_lt_rd = rj_value[31] & ~rkd_value[31]                               // 异号符号位可能溢出
                | ((rj_value[31] ~^ rkd_value[31]) & adder_result[31]);

assign rj_ge_rd = ~rj_value[31] & rkd_value[31]                               // 异号符号位可能溢出
                | ((rj_value[31] ~^ rkd_value[31]) & ~adder_result[31]);

assign rj_ltu_rd = ~adder_cout;

assign rj_geu_rd = adder_cout;  // 从组合逻辑可知，>=比>少一个~rj_eq_rd，和<是不一样的
...
assign br_target = {32{sel_nextpc[1]}} & (pc + br_offs) 
                 | {32{sel_nextpc[2]}} & (rj_value + jirl_offs);
...
assign inst_src_include_rd = inst_bne | inst_beq | inst_st_w | inst_blt | inst_bge | inst_bltu | inst_bgeu | inst_st_b | inst_st_h;
